/**
 * The Server class represents a server that hosts a card game called "War".
 * It handles client connections, game logic, and communication between clients and the server.
 * The server listens on a specified port and waits for two clients to connect before starting the game.
 * During the game, the server manages the decks of cards for each player, handles rounds, and determines the winner.
 * The server also provides methods for checking the end game conditions and handling war situations.
 */
...
/**
 * Runs the server thread to handle client requests.
 */
   public void run() {
    try {
        ...
        // Wrap them in a BufferedReader and PrintWriter for easier handling
...
/**
 * The Server class represents the server side of the card game 'War'.
 * It handles the game logic, communication with clients, and manages the decks of cards.
 */
public class Server {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (clientSocket1.socketIsConnected()) {
            ...
            try {
                ...
                while (true) {
                    if (roundNumber > 0) {
                        // move discard pile to hand
                        player1discardPile.combine(player1hand);
                        player1discardPile.shuffle();
                        player2discardPile.combine(player2hand);
                        player2discardPile.shuffle();
                    }
                    ...
                    String player1Msg = ", P1 have " + player1hand.length() + " cards in hand and "
                            + player1discardPile.length() + " cards in discard pile.\n";
                          
                    String player2Msg = ", P2 have " + player2hand.length() + " cards in hand and "
    ...
    public static boolean whoWins(String player1Card, String player2Card, SocketConnection socket1, SocketConnection socket2) throws IOException, GameOverException {
        ...
        String message =Deck.getString(player1Card)+ "," + Deck.getString(player2Card) + ",\n";
        ...
        if (result < 0) { // if P2's card is higher value
            message = "P2 wins\n\n";
            System.out.print(message);
            socket1.sendMsgToServer(Deck.getString(player1Card) + " , " + Deck.getString(player2Card));
            socket2.sendMsgToServer(Deck.getString(player1Card) + " , " + Deck.getString(player2Card));
    ...
    private static boolean handleWar(SocketConnection socket1, SocketConnection socket2) throws IOException, GameOverException {
        ...
        message = ", P1 have " + player1hand.length() + " cards in hand and "
                + player1discardPile.length() + " cards in discard pile.\n";
        System.out.print(message);
        socket1.sendMsgToServer("Tie! Time for war!");
        ...
        message = ", P2 have " + player2hand.length() + " cards in hand and"
                + player2discardPile.length() + " cards in discard pile.\n";
        System.out.print(message);
        socket2.sendMsgToServer("Tie! Time for war!");
    ...
    checkPlayerCards(cardsNeeded, socket1, socket2, player1hand, player1discardPile, "P1", "P2");
    checkPlayerCards(cardsNeeded, socket1, socket2, player2hand, player2discardPile, "P2", "P1");
}

private static void checkPlayerCards(int cardsNeeded, SocketConnection socket1, SocketConnection socket2, Deck playerHand, Deck playerDiscardPile, String currentPlayer, String otherPlayer) throws IOException, GameOverException {
    String response;
    if (playerHand.length() < cardsNeeded) {
        if ((playerHand.length() + playerDiscardPile.length()) < cardsNeeded) {
            response = "Game over. " + currentPlayer + " ran out of cards and " + otherPlayer + " wins!\n";
            socket1.sendMsgToServer(response);
            socket2.sendMsgToServer(response);
            throw new GameOverException(response);
        } else {
            response = ", Not enough cards. Reshuffling " + currentPlayer + "'s discard pile into their hand\n";
            socket1.sendMsgToServer(response);
            playerHand.combine(playerDiscardPile);
            playerHand.shuffle();
        }
    }
}
